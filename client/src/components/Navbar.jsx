...
const Navbar = () => {
  ...
  const [speakerIcon, setSpeakerIcon] = useState(
    user.settings.autoPlayPreviews
  );
  ...
  useEffect(() => {
    const handleClick = (e) => {
      ...
      while (node.id !== "root") {
        if (
          node.id === "settings-dropdown" ||
          node.id === "settings-hamburger"
        ) {
    ...
    <>
      <div className="nav-container">
        <div className="title-home">
          Rhyth<span id="spotify-title">monica</span>
        </div>

        <div className="nav-search-logout">
          <i
            id="settings-hamburger"
      ...
      {isSettings && (
        <div id="settings-dropdown" className="settings-dropdown">
          <i
            ...
            onClick={handleSetAutoPlayPreviews}
          ></i>
